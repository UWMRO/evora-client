...
function GetTemp({currTemp, setCurrTemp, isDisabled}) {
  ...
  const [dateArray, setDateArray] = useState(() => initialize_array(NUM_OF_DATA_POINTS));
  const [tempDataArray, setTempDataArray] = useState(() => initialize_array(NUM_OF_DATA_POINTS));
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (dateArray.length > NUM_OF_DATA_POINTS) setDateArray(dateArray.slice(1, NUM_OF_DATA_POINTS + 1));
      if (tempDataArray.length > NUM_OF_DATA_POINTS) setTempDataArray(tempDataArray.slice(1, NUM_OF_DATA_POINTS + 1));
    }, graphDelay);
  ...
  const graphButtonHandler = () => {
    setButtonText(buttonText === 'Details' ? 'Hide' : 'Details');
    setGraphDisplay(buttonText === 'Details' ? 'block' : 'none');
  };

  return (
    <fieldset className="nice-fieldset" disabled={isDisabled}>
      ...
      <div className="graphContainer" style={{ display: graphDisplay }}>
        Update Interval (Current: {graphDelay === -1 ? "Disabled" : graphDelay / 1000 + "s"}): 
        ...
        <button onClick={() => setGraphDelay(Number(document.getElementById("delay").value))}>Set</button>
