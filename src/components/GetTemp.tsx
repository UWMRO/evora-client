...
import graph_options from './resources/graph_options.json';
...
function GetTemp({ currTemp, setCurrTemp, isDisabled }) {
  ...
  let tempMessage = (
    <span className='tempMessage'>Current Temperature: -999 °C</span>
  );
    ...
    const interval = setInterval(() => {
      ...
      temperaturePromise.then((val) => {
        ...
        setDateArray((dateArray) => [
          ...dateArray,
          new Date().toTimeString().substring(3, 8),
        ]);
      });
  ...
  if (currTemp != null) {
    tempMessage = (
      <span className='tempMessage'>Current Temperature: {currTemp} °C</span>
    );
  }
  ...
  return (
    <fieldset className='Temperature' disabled={isDisabled}>
      ...
      <button className='temp-set' onClick={callGetTemperature}>
        Get
      </button>
      <button
        className='temp-set'
        onClick={graphButtonHandler}
        style={{ width: '52px' }}
      >
        {buttonText}
      </button>
      ...
      <div className='graphContainer' style={{ display: graphDisplay }}>
        ...
        <select
          onChange={(e) => setGraphDelay(Number(e.target.value))}
          defaultValue={'-1'}
        >
