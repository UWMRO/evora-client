...
function FocusControls() {
  ...
  function handlePlusFocus() {
    if (
      Math.abs(focusDelta) > MAX_FOCUS_DELTA ||
      Math.abs(focusDelta) < MIN_FOCUS_DELTA
    ) {
  ...
  return (
    <fieldset className='manualFocus'>
      ...
      <p>{errorDisplay}</p>
      <br />
      <button className='temp-set minusFocusButton' onClick={handleMinusFocus}>
        -
      </button>
      <input
        type='number'
      ...
      />
      <button className='temp-set' onClick={handlePlusFocus}>
        +
      </button>
    </fieldset>
  );
